...
def create_match():
    if 'user_id' not in session:
        ...
        if request.method == "GET":
            list_user_tuples = User.User.query.with_entities(User.User.username).all()
            list_user = [user[0] for user in list_user_tuples]

            list_user_id_tuples = User.User.query.with_entities(User.User.id).all()
            list_user_id = [user_id[0] for user_id in list_user_id_tuples]

            list_games_tuples = Games.Games.query.with_entities(Games.Games.game_name).all()
            list_games = [game_name[0] for game_name in list_games_tuples]

            list_game_id_tuples = Games.Games.query.with_entities(Games.Games.id).all()
            ...
            new_match = Match.Match(scores=players_scores)
            db.session.add(new_match)
            db.session.flush()

            for i in players_ids:
                if i == players_ids[0]:
                    db.session.add(User_match.User_match(user_id=i, match_id=new_match.id, winner_t_f=True))
                else:
                    db.session.add(User_match.User_match(user_id=i, match_id=new_match.id, winner_t_f=False))

            db.session.add(Game_match.Game_match(games_id=game_id, match_id=new_match.id))
            db.session.commit()
